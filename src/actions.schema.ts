...
export const ActionDrillDownSchema = SchemaRegistry.define({
  ...
  history: {
    "17": (z: TZod) =>
      ActionCommonSchema.forVersion("17")(z).extend({ type: z.literal(EActionTypes.DRILL_DOWN) }),
  },
});
...
const ActionOpenViewCommonSchema = SchemaRegistry.define({
  ...
  history: {
    "17": (z: TZod) =>
      ActionCommonSchema.forVersion("17")(z).extend({ type: z.literal(EActionTypes.OPEN_VIEW) }),
  },
});
