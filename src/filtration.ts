...
export interface IFormulaFilterValue {
   ...
   */
  index?: number;
  /** Тип данных формулы (без учета index) */
  dbDataType: string;
  /** Формат */
  format: EFormatTypes;
  /** Метод фильтрации */
...
export interface IWidgetFiltration {
   ...
   * @param [index] Опциональный индекс элемента массива, если фильтр применялся к конкретному
   * элементу массива результатов формулы.
   *
   * @remarks
   ...
   */
  removeFormulaFilter(formula: string, index?: number): void;
