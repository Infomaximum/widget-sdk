...
describe("BaseWidgetSettingsSchema — парсинг настроек виджета", () => {
  ...
  describe("latest-фабрика (v19)", () => {
    // В v19 (BI-15536) поле filterMode убрано из настроек виджета — режим фильтрации
    // переехал на уровень измерения (WidgetDimensionSchema). Вместо булева ignoreFilters
    // используется enum с наследованием (EWidgetIgnoreFilterMode), default = INHERITED.
    it("filterMode отсутствует в latest-схеме (поле перенесено на уровень измерения)", () => {
      // Arrange
      const schemaLatest = BaseWidgetSettingsSchema(z);

      // Act
      const result = schemaLatest.safeParse({ ...minimalObject });

      // Assert
      expect(result.success).toBe(true);

      if (result.success) {
        expect("filterMode" in result.data).toBe(false);
      }
    });

    it("ignoreFilters по умолчанию INHERITED в latest-схеме", () => {
      // Arrange
      const schemaLatest = BaseWidgetSettingsSchema(z);
      ...
      if (result.success) {
        expect(result.data.ignoreFilters).toBe("INHERITED");
      }
    });

    it("неизвестное значение ignoreFilters отклоняется в latest-схеме", () => {
      // Arrange
      const schemaLatest = BaseWidgetSettingsSchema(z);

      // Act
      const result = schemaLatest.safeParse({ ...minimalObject, ignoreFilters: "NEW_FUTURE" });
      ...
      if (!result.success) {
        ...
        expect(paths).toContain("ignoreFilters");
      }
    });
  });
